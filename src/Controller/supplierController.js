...
const createSupplier = async (req, res) => {
  try {
    if (req.body!== 0) {
      let result = await createQuerys(req,SuppliersModel);
...
const updateSupplier = async (req, res) => {
    try {
      if (req.body && req.body !== 0) {
        let result = await updateQuerys(req, SuppliersModel);
        res.status(200).json({
          status: "success",
          data: result,
        });
...
const listingSupplier = async (req, res) => {
    try {
      let searchRegax={"$regex":req.params.searchkeyword,"$options":"i"}
      let searchArray=[{name:searchRegax}]
      let result= await listingQuerys(req,SuppliersModel,searchArray)
      res.status(200).json(result);
    } catch (error) {
      res.status(500).json({
        status: "failed",
...
const dropdownListSupplier = async (req, res) => {
    try {
      let result= await dropdownQuerys(req,SuppliersModel);
      res.status(200).json({
        status:"success",
        data: result,
      })
...
module.exports = { createSupplier,updateSupplier,listingSupplier,dropdownListSupplier };
