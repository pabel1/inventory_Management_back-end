...
const regiController = async (req, res) => {
  ...
  if (user) {
    ...
    try {
      if (firstname && lastname && email && mobile && photo) {
        let result = await userRegiQuery(req, UserModel);
...
module.exports = { regiController, loginController, loggedUserController,updateProfileController };
